...
if (mysqli_num_rows($result) > 0) {
  ...
  //  $sql = "INSERT INTO donor (name,guardiansname,gender,dob,weight,bloodgroup,organ,email,address,contact,username,password) values ('$name','$guardiansname','$gender','$dob','$weight','$bloodgroup','$organ','$email','$address','$contact', '$username', '$password') SELECT * FROM request ";
    ...
    $sql2 = "INSERT INTO reject (name,gender,dob,weight,bloodgroup,organ,address,contact,bloodqty,collection) SELECT name,gender,dob,weight,bloodgroup,organ,address,contact,bloodqty,collection FROM request WHERE id = $id";
...
if($final){
    echo"Declined";
    header('Location:declineview.php');
}else{
    echo"ERROR!!";
}
}
